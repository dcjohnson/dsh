...
-- UPDATE UPDATE
-- The evaluation of expression lists will use the shunting yard algorithm and an
-- output queue. The push OpCodes will be generated by reading off the output queue.
-- When evaluating an expression, each new value that is the result of an operation,
